...
export function CompanyProfile({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (partnerId) {
      // As of migration 065 this no longer writes to `partners`
      // directly from the client -- it goes through a service-role
      // RPC (partner_update_own_profile) that whitelists exactly these
      // five columns, so a forged request body can't touch `status`
      // or anything else on the row. partnerId itself is never sent;
      // the route resolves it server-side from the session.
      ...
      if (!profileRes.ok) {
        const { error: partnerSyncMessage } = await profileRes.json().catch(() => ({ error: 'unknown error' }));
        setError('บันทึกข้อมูลภายในสำเร็จ แต่ sync ไปหน้าเว็บสาธารณะไม่สำเร็จ: ' + partnerSyncMessage);
        return;
      }
    } else {
      setSaving(false);
