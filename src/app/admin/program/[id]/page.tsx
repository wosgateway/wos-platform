...
export default async function ProgramDetailPage({
  ...
  return (
    <main>
      <Breadcrumb
        trail={[
          ...(partner ? [{ href: `/partners/${partner.id}`, label: partner.name }] : []),
      ...
      <section className="pt-6">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          ...
          <div className="mb-2 flex flex-wrap items-start justify-between gap-4">
            ...
            {pkg.is_promotion ? (
              <span className="rounded-full bg-accent/10 px-3 py-1 text-sm font-semibold text-accent-ink">
                🔥 {t('promotion')}
              </span>
            ) : null}
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-end gap-3 rounded-2xl border border-slate-100 bg-white p-5 card-shadow">
            ...
            {pkg.duration ? (
              <span className="ml-auto text-sm text-slate-400">⏱️ {pkg.duration as string}</span>
            ) : null}
          </div>
